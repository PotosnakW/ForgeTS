    ...
    def train(self):
        ...
        while opt_steps < self.args.max_opt_steps or cur_epoch < self.args.max_epoch:
            ...
            for batch_x in tqdm(self.train_dataloader, total=len(self.train_dataloader)):
                ...
                if loss.item() > 100: # artibtrary value in order to be able to see peakss
                    print(f"step_train_loss:{loss.item()}, observed_mask:{observed_mask.nansum()}, recon_loss:{recon_loss.nansum()}, masked_input: {input_mask.nansum()} , pretrained_mask: {outputs.pretrain_mask.nansum()}, total_loss: {observed_mask.size}")
            ...
            self.logger.alert(
                        title="Saving model",
                        text=f"Saving model after {opt_steps} steps, i.e. end of epoch {cur_epoch}",
                        level=AlertLevel.INFO
                    )
            self.save_model(
                        self.model, path, opt_steps, self.optimizer, self.scaler
