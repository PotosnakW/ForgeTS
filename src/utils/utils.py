...
def _update_inputs(config: Namespace | dict, **kwargs) -> NamespaceWithDefaults:
...
def _validate_inputs(config: NamespaceWithDefaults) -> NamespaceWithDefaults:
    if config.transformer_backbone == "PatchTST" and config.transformer_type != "encoder_only":
        warnings.warn("PatchTST only supports encoder-only transformer backbones.")
        config.transformer_type = "encoder_only"
    if config.transformer_backbone != "PatchTST" and config.transformer_backbone not in SUPPORTED_HUGGINGFACE_MODELS:
        raise NotImplementedError(f"Transformer backbone {config.transformer_backbone} not supported."
    ...
    if config.d_model is None and config.transformer_backbone in SUPPORTED_HUGGINGFACE_MODELS: 
        config.d_model = get_huggingface_model_dimensions(config.transformer_backbone)
        logging.info("Setting d_model to {}".format(config.d_model))
...
class MetricsStorePretrain(NamedTuple):
    train_loss : float = None
    val_loss : float = None
    test_loss : float = None
    finetune_loss_test : float = None
