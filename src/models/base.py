...
class BaseModel(nn.Module):
    ...
    def load_pretrained_weights(run_name : str, 
                                ...
                                beta_to_0: bool = False):
        path = os.path.join(checkpoints_dir, run_name)

        if opt_steps is None:
            opt_steps = [int(i.split('_')[-1].split('.')[0]) for i in os.listdir(path) if i.split('_')[-1].split('.')[0].isdigit()]
        ...
        # set model parameters with beta in name to 0
        if beta_to_0:
            for key in list(checkpoint['model_state_dict'].keys()):
                if 'beta' in key:
                    print("Beta to 0, ", key)
                    checkpoint['model_state_dict'][key] = torch.ones_like(checkpoint['model_state_dict'][key] * (-10))
