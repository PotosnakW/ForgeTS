...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, 
        ...
        Input:
            ...
            mask : [batch_size x n_channels x seq_len]
        Output:
            x : [batch_size x n_channels x n_patches x d_model]
        """

        if mask is None:
            mask = torch.ones(x.shape[0], x.shape[1], x.shape[2], 
                              device=x.device
            )

        mask = Masking.convert_seq_to_patch_view(
            mask, patch_len=self.patch_len).unsqueeze(-1) # mask : [batch_size x n_channels x n_patches x 1]
        mask = mask.repeat_interleave(
            self.d_model, dim=-1) # mask : [batch_size x n_channels x n_patches x d_model]
