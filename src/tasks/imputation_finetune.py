...
from moment.utils.forecasting_metrics import get_imputation_metrics
...
class ImputationFinetuning(Tasks):
    ...
    def validation(self, data_loader, return_preds : bool = False):
        ...
        with torch.set_grad_enabled(self.args.enable_val_grad):
            for batch_x in tqdm(data_loader, total=len(data_loader)):
                ...
                if outputs.pretrain_mask.dim() == 3 and input_mask.dim() == 2:
                    # if channels have different masks, replicate channel number in input mask
    ...
    def compute_imputation_metrics(self, opt_steps: int):
        _, _, (trues, preds, masks) = self.validation(
            self.test_dataloader, return_preds=True)

        metrics = get_imputation_metrics(y=trues, y_hat=preds, reduction='mean', mask=masks)
    ...
    def train(self):
        ...
        if not getattr(self.args, 'randomly_initialize_backbone', False):
            ...
            elif self.args.model_name == 'InfiniMOMENT' and self.args.finetuning_mode =='end-to-end':
                self.load_pretrained_moment(moment_class=MOMENT)
        ...
        while cur_epoch < self.args.max_epoch: # Epoch based learning only
            ...
            if eval_metrics.val_loss < best_validation_loss:
                ...
                imputation_metrics = self.compute_imputation_metrics(opt_steps=opt_steps)
